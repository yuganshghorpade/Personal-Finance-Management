...
const fetchUserDetails = async ( req, res )=>{
    try {
        ...
        const loggedInUser = await User.findById(user.id).select("-password -refreshToken -isVerified -verifyCode -verifyCodeExpiry")
        if (!loggedInUser) {
        ...
        return res.status(500).json({
            success: false,
            statusCode: 500,
            message: "An unexpected error occured while Fetching User Details...",
            errors: [],
            data: null
        });  
    }
